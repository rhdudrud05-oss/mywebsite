...
kpi1 = pd.read_sql("""
    ...
    WHERE 지역='전국' AND 산업='제조업' AND 연월='2026.03'
""", conn).iloc[0,0]

kpi2 = pd.read_sql("""
    SELECT 생산지수 FROM production_index
    WHERE 지역='전국' AND 산업='제조업' AND 연월='2021.12'
""", conn).iloc[0,0]

emp_latest = pd.read_sql("""
    SELECT 취업자수 FROM employment WHERE 지역='계' AND 반기='2025.2/2'
""", conn).iloc[0,0]
...
seoul_2020 = pd.read_sql("""
    SELECT AVG(생산지수) FROM production_index WHERE 지역='서울특별시' AND 산업='제조업' AND 연월 LIKE '2020%'
""", conn).iloc[0,0]
seoul_2025 = pd.read_sql("""
    SELECT AVG(생산지수) FROM production_index WHERE 지역='서울특별시' AND 산업='제조업' AND 연월 LIKE '2025%'
""", conn).iloc[0,0]
...
with c1:
    ...
    <div class="metric-card">
        <div style="color:#94a3b8; font-size:0.8rem;">전국 제조업 생산지수 (2026.03)</div>
...
with c2:
    ...
    st.markdown(f"""
    <div class="metric-card">
        <div style="color:#94a3b8; font-size:0.8rem;">제조업 취업자 (2025 하반기)</div>
...
# 추세선 (연간 평균)
df_c1['year'] = df_c1['연월'].str[:4]
yearly = df_c1.groupby('year')['생산지수'].mean().reset_index()
...
with st.expander("📋 사용된 SQL 및 인사이트"):
    ...
    <div class="sql-box">
    SELECT 연월, 생산지수<br>
    FROM production_index<br>
    WHERE 지역='전국' AND 산업='제조업'<br>
    ORDER BY 연월
    </div>
    <div class="insight-box">
    💡 <b>인사이트:</b> 전국 제조업 생산지수는 2022년 상반기 고점(약 118~122) 이후 2023년 급격히 조정되며
    100 초반까지 하락했습니다. 이는 글로벌 반도체·IT 수요 침체 및 중국 경기 둔화의 직접적 영향입니다.
    2024~2025년 다소 회복되었으나 서울 등 일부 지역은 회복세가 미약하며,
    2026년 초(2~3월)에도 전월 대비 진폭이 큰 불안정한 흐름을 보이고 있습니다.
    </div>
    """, unsafe_allow_html=True)
...
with col_left:
    ...
    df_c2 = pd.read_sql("""
        ...
        ORDER BY avg_2025
    """, conn)
    df_c2['변화율'] = ((df_c2['avg_2025'] - df_c2['avg_2020']) / df_c2['avg_2020'] * 100).round(1)
    ...
    fig2 = go.Figure(go.Bar(
        ...
        text=df_c2['avg_2025'].astype(str),
    ...
    with st.expander("📋 SQL 및 인사이트"):
        ...
        <div class="sql-box">
        SELECT 지역,<br>
        &nbsp;&nbsp;AVG(CASE WHEN 연월 LIKE '2025%' THEN 생산지수 END) as avg_2025<br>
        FROM production_index<br>
        WHERE 산업='제조업' AND 지역 != '전국'<br>
        GROUP BY 지역 ORDER BY avg_2025
        </div>
        <div class="insight-box">
        💡 <b>인사이트:</b> 경기도·인천이 145 이상으로 반도체·첨단 제조 집중 지역의 강세를 보이는 반면,
        <b>서울은 85.6으로 유일하게 기준선(100) 이하</b>로 하락했습니다.
        전통 제조업 기반이 약한 서울은 탈제조업화가 빠르게 진행 중임을 보여줍니다.
        </div>
        """, unsafe_allow_html=True)
...
with col_right:
    ...
    fig3.update_layout(
        ...
        font_color='#cbd5e1', height=460,
        xaxis=dict(showgrid=False, tickangle=-45),
        yaxis=dict(showgrid=True, gridcolor='#334155', title='취업자 수 (천명)',
                   range=[4200, 4620]),
    ...
    with st.expander("📋 SQL 및 인사이트"):
        ...
        <div class="insight-box">
        💡 <b>인사이트:</b> 제조업 취업자는 2022년 4,525천명 고점 이후 지속 감소해
        2025년 하반기 4,364천명으로 161천명이 줄었습니다.
        특히 2025년 상·하반기 연속 4,400천명 이하를 기록하며
        <b>고용 회복 없는 생산 지표 회복</b> — 즉 고용 없는 성장의 구조화를 시사합니다.
        </div>
        """, unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ─── Chart 4: 수출입 Top10 품목 ──────────────────────────────────────────────
st.markdown('<div class="section-title">📦 차트 4: 주요 수출입 품목별 무역수지 (2021.03–2026.03 누계, 단위: 백만 달러)</div>', unsafe_allow_html=True)

df_c4 = pd.read_sql("""
    SELECT 품목명, 수출액/1e6 as 수출액_M, 수입액/1e6 as 수입액_M, 수지/1e6 as 수지_M
    FROM trade
    ORDER BY 수출액 DESC
    LIMIT 12
""", conn)
...
fig4.add_trace(go.Bar(
    name='수출액',
    x=df_c4['품목명_short'], y=df_c4['수출액_M'],
    marker_color='#3b82f6', opacity=0.85,
    hovertemplate='%{x}<br>수출: $%{y:,.0f}M<extra></extra>'
))
fig4.add_trace(go.Bar(
    name='수입액',
    x=df_c4['품목명_short'], y=df_c4['수입액_M'],
...
with st.expander("📋 SQL 및 인사이트"):
    st.markdown("""
    <div class="sql-box">
    SELECT 품목명, 수출액/1e6 as 수출액_M, 수입액/1e6 as 수입액_M, 수지/1e6 as 수지_M<br>
    FROM trade<br>
    ORDER BY 수출액 DESC LIMIT 12
    </div>
    <div class="insight-box">
    💡 <b>인사이트:</b> 전기기기(반도체 포함), 자동차, 기계류가 수출 3대 품목으로 총 수출의 55% 이상을 차지합니다.
    반면 <b>광물성연료는 수출 4위지만 무역수지는 -5,784억 달러</b>로 최대 적자 품목입니다.
    제조업 수출 경쟁력은 유지되지만, 에너지 수입 의존도가 수지 구조를 취약하게 만드는 구조적 문제가 확인됩니다.
    </div>
    """, unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ─── Chart 5: 무역수지 흑·적자 품목 ─────────────────────────────────────────
st.markdown('<div class="section-title">⚖️ 차트 5: 품목별 무역수지 — 흑자 vs 적자 (단위: 백만 달러)</div>', unsafe_allow_html=True)

df_surplus = pd.read_sql("""
    SELECT 품목명, 수지/1e6 as 수지_M
    FROM trade WHERE 수지 > 0 ORDER BY 수지 DESC LIMIT 8
""", conn)
df_surplus['품목명_short'] = df_surplus['품목명'].apply(lambda x: x[:12] + '…' if len(x) > 12 else x)

df_deficit = pd.read_sql("""
    SELECT 품목명, 수지/1e6 as 수지_M
    FROM trade WHERE 수지 < 0 ORDER BY 수지 ASC LIMIT 8
""", conn)
...
fig5.add_trace(go.Bar(
    x=df_surplus['수지_M'], y=df_surplus['품목명_short'],
    orientation='h', marker_color='#22c55e',
    hovertemplate='%{y}<br>흑자: $%{x:,.0f}M<extra></extra>',
    showlegend=False
), row=1, col=1)

fig5.add_trace(go.Bar(
    x=df_deficit['수지_M'], y=df_deficit['품목명_short'],
    orientation='h', marker_color='#ef4444',
    hovertemplate='%{y}<br>적자: $%{x:,.0f}M<extra></extra>',
    showlegend=False
), row=1, col=2)
...
with st.expander("📋 SQL 및 인사이트"):
    ...
    <div class="insight-box">
    💡 <b>인사이트:</b> 흑자 품목은 전기기기(반도체), 자동차, 선박 등 <b>고부가 제조업</b>에 집중되어 있습니다.
    반면 적자 품목은 에너지(광물성연료 -5,784억$), 광물(광슬랙 -1,027억$), 식품·소비재 순으로,
    <b>에너지와 원자재 의존이 제조업 흑자를 상쇄하는 구조</b>임을 알 수 있습니다.
    특히 정밀기기 적자는 핵심부품의 수입의존도가 여전히 높음을 반영합니다.
    </div>
    """, unsafe_allow_html=True)
