...
# ★ 실제 파일명에 맞게 수정 — data/ 폴더 안에 아래 이름으로 저장하세요
PROD_CSV   = "data/시도_산업별_광공업생산지수_2020100__20260511105949.csv"
EMP_CSV    = "data/시도_산업별_취업자_20260511112018.csv"
TRADE_XLS  = "data/품목 수출입 총괄 _ 맞춤통계분석 - K-stat 수출입 무역통계.xls"

def build_db():
    # ── 1. 광공업생산지수 ─────────────────────────────────────────────────────
    df1 = pd.read_csv(PROD_CSV, encoding="cp949", header=0)
    ...
    # ── 2. 취업자 ────────────────────────────────────────────────────────────
    df2 = pd.read_csv(EMP_CSV, encoding="cp949", header=0)
    ...
    # ── 3. 수출입 무역통계 ───────────────────────────────────────────────────
    # ★ .xls 구형 포맷 → engine="xlrd" 필수
    df3 = pd.read_excel(TRADE_XLS, engine="xlrd")
